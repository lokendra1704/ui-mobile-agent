...
class ActionSpace:
    def __init__(self, adb_path="adb", image_width=1080, image_height=2400, normalize_coordinates = True):
        self.adb_path = adb_path
        self.image_width = image_width
        self.image_height = image_height
        self.convert_to_ai_coordinates = normalize_coordinates
    
    def click(self, x, y):
        if self.convert_to_ai_coordinates:
            x = get_screen_x_coordinate(x, self.image_width)
            y = get_screen_y_coordinate(y, self.image_height)
    ...
    def scroll(self, x, y, x1, y1, duration_in_ms=500):
        if self.convert_to_ai_coordinates:
            x = get_screen_x_coordinate(x, self.image_width)
            y = get_screen_y_coordinate(y, self.image_height)
            x1 = get_screen_x_coordinate(x1, self.image_width)
            y1 = get_screen_y_coordinate(y1, self.image_height)
        command = self.adb_path + f" shell input swipe {x} {y} {x1} {y1} {duration_in_ms}"
        print("Issuing Command: ", command)
        subprocess.run(command, capture_output=True, text=True, shell=True)
    ...
    def long_press(self, x, y, duration_in_ms=1000):
        if self.convert_to_ai_coordinates:
            x = get_screen_x_coordinate(x, self.image_width)
            y = get_screen_y_coordinate(y, self.image_height)
        command = self.adb_path + f" shell input swipe {x} {y} {x} {y} {duration_in_ms}"
        print("Issuing Command: ", command)
        subprocess.run(command, capture_output=True, text=True, shell=True)
...
# """)
    ...
    # actionOperator = ActionSpace(normalize_coordinates=False)
    # actionOperator.long_press(1049.9, 545, duration_in_ms=50) # End of the timeline
    # actionOperator.click(1049.8, 545) # t=0 31.88 -> 00:02, 
    # actionOperator.click(50.2, 545) # t=00:01:40
    # actionOperator.long_press(30.07, 545, duration_in_ms=50) # t=19:22
    pass